...
def afficher_post_traitement(uploaded_file=None):
    ...
    if uploaded_file is not None:
        ...
        try:
            df_source = pd.read_excel("data/Urm_Terrain_comparaison.xlsx", sheet_name=None)
        except:
    ...
    df_communes['Différence'] = df_communes['Total Parcelles delimitées et enquetées'] - df_communes[
        'Total Parcelles Terrain']
    df_communes['Pourcentage'] = (df_communes['Total Parcelles delimitées et enquetées'] / df_communes[
        'Total Parcelles Terrain'] * 100).round(1)
    ...
    with tab2:
        ...
        with st.expander("📋 Détails des périodes de travail"):
            ...
            df_details = df_details[
                ['Commune', 'Date début', 'Date fin', 'Jours', 'Levée terrain', 'Rythme journalier']]
    ...
    with tab3:
        ...
        with col3:
            st.metric(
                ...
                delta=f"{commune_data['Pourcentage'] - 100:.1f}%" if commune_data[
                                                                         'Pourcentage'] != 100 else "À l'objectif"
            )
        ...
        if not df_commune_time.empty:
            ...
            df_commune_details['Jours'] = (pd.to_datetime(df_commune_details['Date fin']) - pd.to_datetime(
                df_commune_details['Date début'])).dt.days + 1
            ...
            df_commune_details = df_commune_details[
                ['Date début', 'Date fin', 'Jours', 'Levée terrain', 'Rythme', 'Cumul']]
...
def main():
    ...
    if uploaded_file:
        last_modified = datetime.now().strftime("%d/%m/%Y à %H:%M")
        st.sidebar.info(f"Dernière mise à jour: {last_modified}")
